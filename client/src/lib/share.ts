...
export function buildShareUrl(params: { best: number; rank: number | null; nickname?: string; referrer?: string }) {
  ...
  if (params.referrer) url.searchParams.set("ref", params.referrer); // 추천인
  return url.toString();
}

export async function shareMyRank(opts: { best: number; rank: number | null; nickname?: string; referrer?: string }) {
